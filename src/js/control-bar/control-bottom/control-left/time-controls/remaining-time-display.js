...
//Dependencies required for editability of remaining time  ---  a.video.player
...
class RemainingTimeDisplay extends Component {
  ...
  constructor(player, options){
    ...
    //Allows the remaining time to be edited on focus  ---  a.video.player
  ...
  createEl() {
    let el = super.createEl('div', {
      className: 'vjs-remaining-time vjs-remaining-time-display vjs-time-control vjs-control',
  ...
  addChar(event){
      ...
      if(str.length === 2){
    	...
    	else if( prev.length > 8 || !this.isTimeCharPress(event)){
    	  event.preventDefault();
    	}
      }
  ...
  handleInput(event) {
	...
	if(inputParts.length === 2){
	  let localText = (inputParts)[1].trim();
	  localText = localText.charAt(0) === '-' ? localText.substring(1): localText;
	  let timeParts = localText.split(':');
	  if(timeParts.length > 0 && timeParts.length <= 3){
		let newTime = 0, multiplier = 1;
	    while (timeParts.length > 0) {
	      let arrayElem = timeParts.pop();
	      if(Number.isInteger(Number(arrayElem))){
	    ...
	    this.player_.currentTime(this.player_.duration() - newTime);
  ...
  /**
   * Validates the proper char inputs allowing only digits and ':' and '-' characters ---  a.video.player
   *
   * @method isTimeCharPress
   */
  isTimeCharPress(event){
    if((event.which >= 48 && event.which <= 57) || event.which === 59 || event.which === 109 || event.which === 173 || event.which === 189)
      return true;
    else
      return false;
  }
